...
class FileBrowser(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # ---- Proxy model for filtering by name (search box) ----
        self.proxy = QtCore.QSortFilterProxyModel(self)
        self.proxy.setSourceModel(self.model)
        self.proxy.setFilterCaseSensitivity(QtCore.Qt.CaseInsensitive)
        self.proxy.setFilterKeyColumn(0)  # name column

        # ---- Tree view ----
        self.tree = QtWidgets.QTreeView()
        self.tree.setModel(self.proxy)
        source_root = self.model.index(QtCore.QDir.homePath())
    ...
    def open_file(self, index):
        """Emit file path when user double-clicks a file (proxy-aware)."""
        # Map from proxy index back to source model
    ...
    def refresh_view(self):
        ...
        if hasattr(self, "proxy"):
            source_root = self.model.index(current_root)
            self.tree.setRootIndex(self.proxy.mapFromSource(source_root))
        else:
    ...
    def delete_item(self):
        ...
        # Map from proxy index to source model index if proxy exists
        if hasattr(self, "proxy"):
            source_index = self.proxy.mapToSource(index)
        else:
            source_index = index
    ...
    def on_filter_text_changed(self, text):
        """Filter the tree view by file/dir name using the proxy model."""
        if hasattr(self, "proxy"):
            # Show everything when text is empty; otherwise filter by wildcard
